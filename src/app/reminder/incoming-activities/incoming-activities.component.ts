...
export class IncomingActivitiesComponent implements OnInit {
  ...
  constructor(private servicioNotificaciones: RecordatoriosService,private storage: StorageService) {}

  ngOnInit(): void {

     this.usuarioId  = this.storage.obtenerUsuario().usuario_id
    this.servicioNotificaciones.getNotificacionesProximas(this.usuarioId).subscribe(
      (data) => {
